...
export default function App() {
 ...
 async function bindProject(project) {
  ...
  // Bind the selected project to the EXISTING new-session tab.
  // Do not create another tab here.
  patchTab(id, {
    project,
    title: project.name || id.slice(0, 8),
    tree: null,
  ...
  try {
    ...
    // Fetch files directly for this session instead of calling refreshFiles()
    // while the current render may still contain the old tab state.
    try {
      const filesRes = await fetch(`/api/sessions/${id}/files`);
      if (filesRes.ok) {
        const filesData = await filesRes.json();
        patchTab(id, { tree: filesData.tree });
      }
    } catch {
      // File refresh can be retried when the active tab changes.
    }
  } catch (error) {
    patchTab(id, {
      project: null,
  ...
  async function refreshFiles() {
    ...
    try {
      ...
      // Use the selected file from the target tab, not the stale render closure.
      if (target.selected) {
        loadFile(target.selected, id);
      }
    } catch {
      // ignore transient refresh errors
    }
  ...
  async function loadFile(path, sessionId = activeIdRef.current) {
    const id = sessionId;
    patchTab(id, {
      selected: path,
      fileContent: "",
